...
class Index extends Component{
    ...
    render(){
        ...
        for(var k=0;k<j;k++){
            children.map((item,i)=>{
                sliderDom.push(<div key={10*i+k} className={styles.swiper_slide} style={this.state.index%children.length==i&&slide_style_active||slide_style}>
                    {item}
                </div>)
            })
        }
        return(
            <div className={styles.swiper_container} ref='swiper' onTouchStart={this.handleTouchStart} onTouchMove={this.handleTouchMove} onTouchEnd={this.handleTouchEnd}>
                <div className={styles.swiper_wrapper} style={wrapper_style} >
                    {sliderDom}
                </div>
            </div>
        )
