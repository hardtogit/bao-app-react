export const BANNER_LIST='BANNER_LIST'
export const FETCH_ACTIVE_LIST='FETCH_ACTIVE_LIST'
export const FETCH_ACTIVE_MASSAGE='FETCH_ACTIVE_MASSAGE'
export const USER_LOGIN_FLOW='USER_LOGIN_FLOW'
export const FETCH_DIRECTLIST_DATA='FETCH_DIRECTLIST_DATA'
export const FETCH_CREDITORSLIST_DATA='FETCH_CREDITORSLIST_DATA'
export const USER_INFO='USER_INFO'
export const RATE='RATE'
export const VERIFY_CAPTCHA='VERIFY_CAPTCHA'
export const CHECK_VERIFY_CAPTCHA_W='CHECK_VERIFY_CAPTCHA_W'
export const FIND_PWD_SET='FIND_PWD_SET'
export const DEPOSIT_DETAILS='DEPOSIT_DETAILS'
export const GET_BANK_LIST='GET_BANK_LIST'
export const RELATED_PROJECTS='RELATED_PROJECTS'
export const EXIST_MOBILE='EXIST_MOBILE'
export const REGISTER_NUM='REGISTER_NUM'
export const CHECK_VERIFY_CAPTCHA='CHECK_VERIFY_CAPTCHA'
export const REGISTER='REGISTER'
export const DIRECTINVEST_DETAIL='DIRECTINVEST_DETAIL'
export const FETCH_DIRECT_BUY_RECORD_DATA='FETCH_DIRECT_BUY_RECORD_DATA'
export const SAFE_CARD_INFO='SAFE_CARD_INFO'
export const WE_CHAT='WE_CHAT'
export const LOGIN_OUT='LOGIN_OUT'
export const SITE_LIST='SITE_LIST'
export const DEPOSITBS_PLANB='DEPOSITBS_PLANB'
export const DEPOSITBS_DETAILS='DEPOSITBS_DETAILS'
export const NEW_USER_ACTIVITY='NEW_USER_ACTIVITY'
export const GET_CAPTCHA='GET_CAPTCHA'
export const DEPOSITS_GATHER='DEPOSITBS_GATHER'
export const DEPOSITS_GATHER_INDEX='DEPOSITBS_GATHER_INDEX'
export const STORE_STATUS_INFO='STORE_STATUS_INFO'//存管开通状态
export const GATHER_DETAIL='GATHER_DETAIL' //聚点+详情
export const GATHER_PROBLEMS='GATHER_PROBLEMS' //聚点+问题
export const GATHER_BID_LIST='GATHER_BID_LIST' //聚点+标
export const GATHER_JOIN='GATHER_JOIN'//加入记录
export const GATHER_BID_DETAIL='GATHER_BID_DETAIL'//标的详情
export const GATHER_CONTRACT='GATHER_CONTRACT'//聚点+合同
export const DEPOSITS_YOU='DEPOSITS_YOU'
export const DEPOSITS_YOU_INDEX='DEPOSITS_YOU_INDEX'
export const YOU_DETAIL='YOU_DETAIL' //聚点+详情
export const YOU_PROBLEMS='YOU_PROBLEMS' //聚点+问题
export const YOU_BID_LIST='YOU_BID_LIST' //聚点+标
export const YOU_JOIN='YOU_JOIN'//加入记录
export const YOU_BID_DETAIL='YOU_BID_DETAIL'//标的详情
export const YOU_CONTRACT='YOU_CONTRACT'//聚点+合同
export const BUY_DETAIL='BUY_DETAIL' //聚点+详情
export const BUY_PROBLEMS='BUY_PROBLEMS' //聚点+问题
export const BUY_BID_LIST='BUY_BID_LIST' //聚点+标
export const BUY_JOIN='BUY_JOIN'//加入记录
export const BUY_BID_DETAIL='BUY_BID_DETAIL'//标的详情
export const BUY_CONTRACT='BUY_CONTRACT'//聚点+合同
export const IF_SYNC='IF_SYNC'//是否同步
export const AUTH_COOKIE='AUTH_COOKIE'//獲取auth
export const GET_GOODS_LIST='GET_GOODS_LIST'
export const GET_VIP='GET_VIP'
export const GET_PRIV_BASIC='GET_PRIV_BASIC'
export const GET_RATE_COUPONS='GET_RATE_COUPONS'
export const GET_VOUCHER_COUPONS='GET_VOUCHER_COUPONS'
export const GET_HOT_ACTIVITY='GET_HOT_ACTIVITY'
export const GET_GOODS_TYPE_LIST='GET_GOODS_TYPE_LIST'
export const GET_GOODS_LIST_NEW='GET_GOODS_LIST_NEW'
export const GET_GOODS_LIST_NEW_ONE='GET_GOODS_LIST_NEW_ONE'
export const GET_GOODS_LIST_NEW_TWO='GET_GOODS_LIST_NEW_TWO'
export const GET_FIND_BANNER='GET_FIND_BANNER'//发现Banner
export const GET_MALL_BANNER='GET_MALL_BANNER'//商城Banner
export const GET_DEFAULT_TAB='GET_DEFAULT_TAB'//获取默认tab
export const GET_PREHEAT_INFO='GET_PREHEAT_INFO' //预热活动列表
export const PREHEAT_RECEIVE='PREHEAT_RECEIVE' //预热活动列表
export const PREHEAT_STATUS='PREHEAT_STATUS' //预热活动列表
export const GET_CHARGE_STATUS='GET_CHARGE_STATUS'//回去新老用户标识
export const GET_HELP_LIST='GET_HELP_LIST'//商城帮助信息
export const PRODUCT_DETAIL='PRODUCT_DETAIL'//商城帮助信息
export const GET_NOTICE_LIST='GET_NOTICE_LIST'//商城通知
export const GET_NOTICE_DETAIL='GET_NOTICE_DETAIL'//商城通知
export const GET_PRIV_PROFIT='GET_PRIV_PROFIT'//商城通知
export const GET_VIP_STR='GET_VIP_STR'//商城通知
export const WISDOM_LIST='WISDOM_LIST'//聚点债转列表
export const WISDOM_DETAIL='WISDOM_DETAIL'//聚点债权详情
export const WISDOM_INVEST_LIST='WISDOM_INVEST_LIST'//投资人详情
export const FETCH_LIST_DATA_FAIL='FETCH_LIST_DATA_FAIL'//获取数据失败